...
I create the final fitbit-based data object called `data_fitbit` by removing the `idstudy` column and duplicate observations. The following code presents the first ten rows of this new object and information on the dimensions.
...
## Dyadic activity information

The `data_dyad_steps` and `data_dyad_sleep` objects contain several variables: the `participid` for each vertex, the observation date, the activity value for each vertext (suffixed by `_v1` and `_v2`), and the absolute value of the difference between the two activity values (`abs_diff`).[^1]

[^1]: This is simply to account for the fact that vertex one or two might have a higher value. The direction of the difference (positive or negative) is not an important consideration at this point.
